...
class BettingModel:
    def __init__(self, input_len):
        ...
        self.params = np.random.randn(2 * self.half_params) * 0.1
    ...
    def _predict_amounts(self, input_data, odds_home, odds_away):
        ...
        """
        # Split parameters into home and away components
        home_params = self.params[:self.half_params]
        away_params = self.params[self.half_params:]

        # Create feature vectors
        home_X = np.column_stack([input_data, odds_home])
        away_X = np.column_stack([input_data, odds_away])

        # Calculate logits for home and away bets
        home_decision = (expit(home_X @ home_params[2:] + home_params[1]) * odds_home - 1) * home_params[0]
        away_decision = (expit(away_X @ away_params[2:] + away_params[1]) * odds_away - 1) * away_params[0]

        # Apply sigmoid to get bounded bet amounts
        bet_amt_home = expit(home_decision)
        bet_amt_away = expit(away_decision)

        return bet_amt_home, bet_amt_away

    def _loss_function(self, params, input_data, odds_home, odds_away, outcomes):
        """
        ...
        home_returns = (odds_home * outcomes - 1) * bet_amt_home
        away_returns = (odds_away * (1 - outcomes) - 1) * bet_amt_away

        pnl = np.sum(home_returns + away_returns)

        print('it:', pnl)

        return -pnl

    def fit(self, input_data, odds_home, odds_away, outcomes):
        print('\n\nMinimizing...')

        result = minimize(
            fun=self._loss_function,
            x0=self.params,
            args=(input_data, odds_home, odds_away, outcomes),
            method='BFGS',
    ...
    def predict(self, input_data, odds_home, odds_away):
        return self._predict_amounts(input_data, odds_home, odds_away)
